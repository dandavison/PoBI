...
#+begin_src R :var chrom=22 :var nmax=20
  ...
  for(pop2 in c("Orkney","Spain")) {
      ...
      for(i in 1:min(nmax,ncol(h1))) {
          cat(i, " ")
          id.lengths[[pop1]][[pop2]][[i]] <- list()
          for(j in 1:min(nmax,ncol(h2)))
              id.lengths[[pop1]][[pop2]][[i]][[j]] <- identical.stretch.lengths(h1[,i], h2[,j])
