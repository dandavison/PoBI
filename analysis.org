...
#+begin_src R
  ...
  g.hapfile <- function(pop, chrom)
      file.path("garrett", "haplotypes", pop, paste(chrom, ".haps", sep=""))
...
** Stitch haplotypes
#+begin_src R :var pops=sample-sizes :results output replace
print(str(pops))
#+end_src
...
:  $ V1: chr  "Belgium" "Denmark" "France" "Germany" ...
:  $ V2: int  544 332 479 1100 61 745 953 58 205 1212 ...
: NULL

#+begin_src R :var pops=sample-sizes :var nolap=101
  for(pop in c("Poland")) { ## pops[[1]]
...
#+begin_src R :var chroms='(22) :var pops='(UK) :results output replace
  ...
  write.garrett.haplotype.file <- function(haps, pop, chrom)
      cat(rbind(haps, rep("\n", ncol(haps))),
          sep="", file=g.hapfile(pop, chrom))
  ...
  for(c in chroms) {
      ...
      for(pop in pops) {
          ...
          write.garrett.haplotype.file(haps, pop, c)
