...
#+begin_src R
  ...
  chroms <- 1:22
  nwtccc2 <- 8267
  wfile <- function(chr) sprintf("data/POBI+MS/%02d-allx.gen", chr)
  wrfile <- function(chr) sprintf("POBI+MS-%02dr.gen", chr)
  wflippedfile <- function(chr) sprintf("data/POBI+MS/%02d-allx-flipped.gen", chr)
  wlinesfile <- function(chr) sprintf("%02d.lines", chr)
  wfreqfile <- function(chr) sprintf("%02d-w.freq", chr)
  
  hapfile <- function(chr) sprintf("data/HM3/CEU+TSI.chr%d.hap", chr)
  hlinesfile <- function(chr) sprintf("%02d.h.lines", chr)
  hfreqfile <- function(chr) sprintf("%02d-h.freq", chr)
  hfile <- function(chr) sprintf("data/HM3/hapmap3.r2.b36.chr%d.legend", chr)
  hrfile <- function(chr) sprintf("hapmap3-%02dr.gen", chr)
...
#+begin_src R :results silent
  ...
  names(h) <- names(w) <- sprintf("%02d", chroms)
...
#+begin_src sh
# wfile(22)  
sstat -p -n 8267 < data/POBI+MS/22-allx-flipped.gen > 22-w.freq
#+end_src
...
#+begin_src R
  ...
  for(c in 22) {
      ...
      png(file=sprintf("%02d-freqs.png", c))
