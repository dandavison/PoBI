...
***** Recipient analysis
#+begin_src R
  total.length <- counts <- mean.lengths <- list()
  
  
  for(c in chroms) {
      counts[[c]] <- chromopainter.read.recipient.chunk.counts(c, dir=cpdir, exclude=exclude)
      total.lengths[[c]] <- chromopainter.read.recipient.chunk.lengths(c, dir=cpdir, exclude=exclude)
  ...
  for(c in chroms) {
      labs <- sapply(px[[c]], nrow)
      labs <- factor(rep(names(labs), labs))
      ## No donor pooling
      pdf(sprintf("img/qplot-%s.pdf", c), width=20, height=5)
      psi.qplot(px1[[c]], labels=labs)
      dev.off()
      ## DGNS pooling
