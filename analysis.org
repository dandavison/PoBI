...
#+begin_src R
  ...
  hapfile <- function(chr, dataset=c("h","w"))
      switch(match.arg(dataset),
             ...
             h = sprintf("data/HM3/%s.hap", chr))
  henfile <- function(chr, pop)
      file.path("impute2", pop, "out", chr, "all.hen")
...
#+begin_src R :var chunk=4000000
  ...
  for(pop in c(levels(indivs.d$reg2), "All")) {
      ...
      for(c in chroms) {
          ...
          intervals <- make.intervals(pos, width=chunk, overlap=100)
...
** Stitch haplotypes
#+begin_src R
  done.pops <- c("Denmark","France","Ireland","Norway","Poland","Spain","UK")
#+end_src

#+begin_src R :var pops=sample-sizes :var nolap=101
  for(pop in done.pops) {
      ...
      for(c in "22") {
          ...
          stitch.haplotypes(hapfiles, ids, file.path(outdir, "all.hen"), nolap=nolap)
      }
  }
#+end_src
...
#+begin_src R :var codes.d=region-codes :results output replace
  chroms <- "22"
  pops <- done.pops ## levels(indivs.d$reg2)
  ...
  write.garrett.haplotype.and.id.files <- function(haps, ids, pos, chrom, pop, perm) {
      ...
      hap.perm <- c(rbind(2*perm - 1, 2*perm))
      haps <- haps[,hap.perm]
      ids <- ids[perm]
  ...
  set.seed(14011978)
  perms <- sapply(levels(indivs$reg), function(reg) sample(1:sum(indivs.d$reg == reg)),
                  simplify=FALSE)
  names(perms) <- codes[names(perms)]
  
  for(c in chroms) {
      leg0 <- read.chiamo.legend(henfile(c, "Norway"))
      ...
      for(pop in pops) {
          ...
          if(pop != "UK")
              write.garrett.haplotype.and.id.files(haps, ids, pos=leg$pos, chrom=c, pop=codes[pop], perm=perms[pop])
          else for(UKreg in levels(indivs.d$reg)) {
              inreg <- indivs.d$reg == UKreg
              haps.reg <- haps[,inreg,drop=FALSE]
              ids.reg <- ids[inreg]
              hapfile <- 
              write.garrett.haplotype.and.id.file(haps.reg, ids.reg, pos=leg$pos, chrom=c, pop=codes[UKreg], perm=perms[pop])
              cat(levels(ids)[ids.reg], sep="\n", file=g.idfile(codes[UKreg]))
          }
      }
  }
