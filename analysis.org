...
#+begin_src R :var chunk=4000000
  make.intervals <- function(points, width, noverlap=100) {
      ## Return intervals with at least noverlap overlapping points at each end
      stop("unfinished")
      ...
      while(TRUE) {
          ...
          n <- sum(points >= starts[i] & points <= ends[i])
      }
