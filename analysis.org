...
#+begin_src R
  insect.chroms <- c("16", "16.0", "17", "18", "18.0", "19", "20", "20.0", "21", "22", "22.0")
  pops <- unique(as.character(indivs.d$reg2))
  ...
  for(c in insect.chroms) {
      pop.files <- sapply(pops, lf)
      if(!is.matrix(pop.files)) {
          cat("Different numbers of chunk files across pops:\n")
...
#+begin_src R :var pops=sample-sizes[,0]
  ...
  for(pop in pops) {
      ...
      for(c in names(chrom.nchunks)) {
          hapfiles <- list.files(path=file.path("insect", c),
                                 pattern=sprintf("%s_%s_[0-9][0-9][0-9]_haps.insect", pop, c),
...
***** R environment
#+begin_src R
  chroms <- as.character(c(18,20,22))
  SCA <- c("DEN","SWE","NOR")
#+end_src
...
#+begin_src R :var codes.d=region-codes :results output replace
  ...
  for(c in chroms) {
      ...
      for(pop in pops) {
          ...
          haps <- NULL ## read.haplotypes(hen, ids)
          if(pop != "UK")
              ...
              for(UKreg in UKregions) {
                  ...
                  haps.reg <- NULL ## haps[,rep(inreg, each=2),drop=FALSE]
                  ids.reg <- ids[inreg]
                  hapfile <-
...
#+begin_src R
  ...
  stopifnot(unlist(lapply(counts, names)) %in% pops)
  stopifnot(unlist(lapply(total.length, names)) %in% pops)
  ...
  for(c in chroms) {
      ...
      ## No donor pooling
      pdf(sprintf("img/qplot-meanlengths-%s.pdf", c), width=20, height=5)
      psi.qplot(mean.lengths1[[c]], labels=labs)
      dev.off()
      pdf(sprintf("img/qplot-totallength-%s.pdf", c), width=20, height=5)
      psi.qplot(total.lengths1[[c]], labels=labs)
      dev.off()
      pdf(sprintf("img/qplot-counts-%s.pdf", c), width=20, height=5)
...
- The top copiers of Scandinavia (Denmark, Sweden, Norway) are
  Lancashire, North Pembrokeshire, Orkney, Yorkshire
